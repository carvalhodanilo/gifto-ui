...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  const { setTenantFromSelection } = useTenant();
  ...
  const applyClaimsToContexts = React.useCallback(
    (tokenParsed: unknown) => {
      ...
      if (tenantId) setTenantFromSelection({ id: tenantId, fantasyName: tenantId });
      if (merchantId) setMerchant({ id: merchantId, merchantName: merchantId });

      return { tenantId, merchantId };
    },
    [setTenantFromSelection, setMerchant]
  );
  ...
  // Resolve o nome do merchant (para não mostrar o UUID no header).
  React.useEffect(() => {
    if (!state.authenticated || !state.merchantId) return;
    let cancelled = false;

    getMerchantById(state.merchantId)
      .then((detail) => {
        if (cancelled) return;
        const name = detail.merchantName ?? detail.fantasyName ?? detail.merchantId;
        setMerchant({ id: detail.merchantId, merchantName: name });
      })
      .catch(() => {});
  ...
  }, [state.authenticated, state.merchantId, setMerchant]);
